...
class Chat {
public:
    Chat(const User &sender,const  User &receiver) {
        user1=sender;
        user2=receiver;
    };
    Chat( const std::vector<Message> &message, const User &sender, const User &receiver):
    ...
    void set_message(const std::vector<Message> &message) { //test
        chat_m=message;
